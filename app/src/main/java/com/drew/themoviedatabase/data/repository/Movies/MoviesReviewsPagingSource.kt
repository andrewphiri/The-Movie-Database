...
class MoviesReviewsPagingSource(
    val moviesRepository: MovieRepository,
    val movieId: Int
): PagingSource<Int, com.drew.themoviedatabase.data.model.Reviews>() {
    override fun getRefreshKey(state: PagingState<Int, com.drew.themoviedatabase.data.model.Reviews>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, com.drew.themoviedatabase.data.model.Reviews> {
        return try {
            // Key may be null during a refresh, if no explicit key is passed into Pager
            // construction. Use 1 as default, because our API is indexed started at index 1
            val pageNumber = params.key ?: 1

            val popularMovies = moviesRepository.fetchReviews(movieId = movieId, page = pageNumber)
